...
## Worked Example: Database Selection for Non-Life Insurance Claims System

**Context**: A non-life insurance company needs a claims management system handling:
- 500K active claims, growing 20%/year
- 50 claims adjusters + 200 agents creating/updating claims
- Claim lifecycle: FNOL → Investigation → Assessment → Approval → Settlement → Closed
- Each claim has 5-50 documents (photos, PDFs, reports)
- Regulatory requirement: 10-year retention, full audit trail, reconciliation with finance
- Reporting: daily operational dashboards + monthly regulatory reports

**Access pattern inventory**:

| Operation | Pattern | Frequency | Latency target | Consistency |
|---|---|---|---|---|
| Create claim (FNOL) | Insert claim + initial documents | 200/day | < 500ms | Strong |
| Update claim status | Update with state transition guard + audit | 2000/day | < 300ms | Strong |
| View claim detail | Point read by claim_id with related entities | 5000/day | < 200ms | Read-your-writes |
| Search claims (adjuster) | Filter by status, assignee, date range, LOB | 3000/day | < 1s | Eventual (30s lag OK) |
| Claim documents list | List by claim_id, ordered by upload date | 2000/day | < 300ms | Strong |
| Regulatory report | Aggregate by LOB, status, period, region | 5/day | < 30s | Eventual (1h lag OK) |
| Audit trail query | Filter by claim_id, actor, date range | 100/day | < 2s | Strong |
| Reconciliation | Compare claim totals vs finance ledger | 1/day | < 5min | Eventual |

**Decision matrix**:

| Option | Fit | Rejected because |
|---|---|---|
| **PostgreSQL (chosen)** | Strong OLTP fit: ACID transactions for state transitions, constraints for lifecycle guards, JSON for flexible claim attributes, excellent indexing, mature tooling, 10-year retention with partitioning | — |
| MongoDB | Document model fits claim aggregate shape | Cross-claim queries (reporting, reconciliation) are weaker; transaction support is newer and less battle-tested for financial correctness; regulatory audit trail harder to enforce with constraints |
| DynamoDB | Scales writes infinitely | Access patterns are too varied (ad-hoc search, reporting, joins for reconciliation); GSI limitations; cost at 500K items with complex queries; team has no DynamoDB expertise |
| SQL Server | Strong enterprise OLTP | License cost; team expertise is PostgreSQL; no technical advantage for this workload |

**Derived stores**:

| Store | Purpose | Sync method | Freshness | Rebuild |
|---|---|---|---|---|
| Elasticsearch | Adjuster search (status, assignee, date, LOB, text) | CDC via Debezium → Kafka → indexer | ≤ 30s | Full reindex from PostgreSQL (2h for 500K claims) |
| S3 | Claim documents (photos, PDFs) | Direct upload via signed URL; metadata in PostgreSQL | Immediate | N/A (source of truth for bytes) |
| PostgreSQL read replica | Regulatory reporting, reconciliation | Streaming replication | ≤ 5s | Automatic |
| Redis | Session cache, rate limit counters, claim-lock for concurrent edit prevention | Application-managed | N/A (ephemeral) | Warm from DB on miss |

**Physical design (PostgreSQL)**:

```sql
-- Partition claims by year for retention management
CREATE TABLE claims (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  claim_number    TEXT NOT NULL UNIQUE,
  tenant_id       UUID NOT NULL,
  policy_id       UUID NOT NULL,
  status          TEXT NOT NULL CHECK (status IN ('FNOL','INVESTIGATION','ASSESSMENT','APPROVAL','SETTLEMENT','CLOSED','REJECTED')),
  ...
  -- ... other fields
  CONSTRAINT valid_transition CHECK (/* enforced via trigger or app */)
) PARTITION BY RANGE (created_at);

-- Indexes aligned with access patterns
CREATE INDEX ix_claims_assignee_status ON claims (assignee_id, status) WHERE status NOT IN ('CLOSED','REJECTED');
CREATE INDEX ix_claims_tenant_status ON claims (tenant_id, status, created_at DESC);
...
CREATE TABLE claim_audit (
  ...
  actor_type  TEXT NOT NULL,  -- 'adjuster', 'system', 'customer'
  old_state   JSONB,
  new_state   JSONB,
  reason      TEXT,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
...
**Why this works**:
- PostgreSQL handles the OLTP workload (200 writes/day is trivial; even 20K/day is comfortable).
- Partitioning by year enables efficient retention management (drop old partitions after 10 years).
- Elasticsearch handles the varied search patterns that would require expensive full-text + filter queries on PostgreSQL.
- Read replica isolates reporting load from OLTP.
- Redis handles ephemeral concerns (sessions, locks) without polluting the data model.
- Clear source-of-truth: PostgreSQL for claim state, S3 for document bytes, Elasticsearch for search (derived, rebuildable).
