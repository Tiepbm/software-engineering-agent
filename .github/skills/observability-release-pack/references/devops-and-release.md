...
## Worked Example: Canary Rollout with SLO Gates (Kubernetes + Argo Rollouts)

**Scenario**: Deploy a new version of the `claims-api` service that changes claim status transition logic. High-risk because incorrect transitions can corrupt claim state and trigger compliance issues.

**Rollout definition** (Argo Rollouts):
        ...
        istio:
          virtualService:
            ...
            routes:
              - primary
      steps:
        # Step 1: 5% traffic for 10 minutes — catch obvious crashes
        - setWeight: 5
        - pause: { duration: 10m }
        # Step 2: Automated SLO check
        - analysis:
            templates:
              - templateName: claims-api-slo-check
            args:
              - name: canary-hash
                valueFrom:
                  podTemplateHashValue: Latest
        # Step 3: 25% traffic for 15 minutes
        - setWeight: 25
        - pause: { duration: 15m }
        # Step 4: Second SLO check at higher traffic
        - analysis:
            templates:
              - templateName: claims-api-slo-check
            args:
              - name: canary-hash
                valueFrom:
                  podTemplateHashValue: Latest
        # Step 5: 50% for 15 minutes
        - setWeight: 50
        - pause: { duration: 15m }
        # Step 6: Final SLO check before full promotion
        - analysis:
            templates:
              - templateName: claims-api-slo-check
            args:
...
metadata:
  name: claims-api-slo-check
spec:
  args:
    - name: canary-hash
  metrics:
    # Gate 1: Error rate must stay below 0.5%
    - name: error-rate
      interval: 2m
      count: 3
      ...
      provider:
        prometheus:
          ...
          query: |
            sum(rate(http_requests_total{service="claims-api",
              pod_template_hash="{{args.canary-hash}}", status=~"5.."}[5m]))
            /
            sum(rate(http_requests_total{service="claims-api",
              pod_template_hash="{{args.canary-hash}}"}[5m]))

    # Gate 2: p99 latency must stay below 800ms
    - name: latency-p99
      interval: 2m
      count: 3
        ...
        prometheus:
          ...
          query: |
            histogram_quantile(0.99,
              sum(rate(http_request_duration_seconds_bucket{service="claims-api",
                pod_template_hash="{{args.canary-hash}}"}[5m])) by (le))

    # Gate 3: No claim state corruption (business metric)
    - name: claim-transition-errors
      interval: 2m
      count: 3
      successCondition: result[0] == 0
      failureLimit: 0  # Zero tolerance for state corruption
      provider:
        prometheus:
          address: http://prometheus:9090
          query: |
            sum(increase(claim_invalid_transition_total{
              pod_template_hash="{{args.canary-hash}}"}[5m])) or vector(0)
```

**What happens on failure**:
1. Analysis metric fails → Argo Rollouts automatically aborts.
2. Traffic routes back to stable (100% to old version) within seconds.
3. Canary pods scale down after `abortScaleDownDelaySeconds`.
4. Alert fires: `canary_rollout_aborted{service="claims-api"}` → on-call investigates.
5. Team reviews canary logs, traces, and the specific metric that failed.
6. Fix → new image → re-trigger rollout.

**Pre-deploy checklist**:
- [ ] Smoke tests pass on staging with same image.
- [ ] Database migration (if any) already applied and compatible with both old and new code.
- [ ] Feature flag for new transition logic defaults to OFF; canary enables it.
- [ ] Rollback tested: previous image still works with current DB schema.
- [ ] On-call aware of the deploy window and abort criteria.
- [ ] `claim_invalid_transition_total` metric exists and emits from the new code.

**Post-promotion verification** (next 2 hours):
- [ ] Error budget burn rate normal.
- [ ] No increase in claim support tickets.
- [ ] Reconciliation job shows no orphaned or invalid claim states.
- [ ] Queue lag (claim events) stable.
- [ ] Partner callback success rate unchanged.
